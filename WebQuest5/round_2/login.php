<?php
  ...
  if (isset($_POST['submit'])) {
    ...
    $conn = new mysqli($host, $username, $password, $dbname);
      ...
      $db_query = 'select password, payment from wq2019_players where username="'.$player_username.'"'  ;
      ...
      while ( $row = mysqli_fetch_assoc($result)){
        ...
        if ( $row['payment'] == '0' || $row['payment'] == 0 ){
          session_destroy();
          echo "<script>window.location = 'login.php?payment=true';</script>";
        }
...
<body>
  ...
  <script>
      ...
      ?>){
        alert('You have not done payment for round 2');
      }
